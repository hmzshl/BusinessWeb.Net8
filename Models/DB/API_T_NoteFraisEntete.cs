// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
#nullable disable
using System;
using System.Collections.Generic;

namespace BusinessWeb.Models.DB;

public partial class API_T_NoteFraisEntete
{
    public int id { get; set; }

    public DateTime? Date { get; set; }

    public string Numero { get; set; }

    public string Libelle { get; set; }

    public string CT_Num { get; set; }

    public string CA_Num { get; set; }

    public int Personnel { get; set; }

    public int Materiel { get; set; }

    public string MontantLettre { get; set; }

    public int Type { get; set; }

    public int NoteFrais { get; set; }

    public decimal Montant { get; set; }

    public int Site { get; set; }

    public int Projet { get; set; }

    public int Sense { get; set; }

    public int Affectation { get; set; }

    public int Representant { get; set; }

    public string Remarque { get; set; }

    public string Reference { get; set; }

    public DateTime? Creation { get; set; }

    public DateTime? Modification { get; set; }

    public string CreationIP { get; set; }

    public string ModificationIP { get; set; }

    public string CreationHost { get; set; }

    public string ModificationHost { get; set; }

    public string CreationUser { get; set; }

    public string ModificationUser { get; set; }

    public bool Valide { get; set; }

    public DateTime? ValideDate { get; set; }

    public virtual ICollection<API_T_NoteFraisLigne> API_T_NoteFraisLigne { get; set; } = new List<API_T_NoteFraisLigne>();
}