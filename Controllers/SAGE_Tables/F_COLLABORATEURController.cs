...
namespace BusinessWeb.Controllers.SAGE_Tables
{
    ...
    public partial class F_COLLABORATEURController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<F_COLLABORATEUR>>> GetF_COLLABORATEUR([FromQuery] int page = 1, [FromQuery] int pageSize = 100)
        {
            int skip = (page - 1) * pageSize; pageSize = Math.Min(pageSize, 500); setDB(); return await _db.F_COLLABORATEUR.AsNoTracking().Skip(skip).Take(pageSize).ToListAsync();
        }
