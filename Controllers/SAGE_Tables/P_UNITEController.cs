...
namespace BusinessWeb.Controllers.SAGE_Tables
{
    ...
    public partial class P_UNITEController : ControllerBase
    {
        ...
        // GET: api/P_UNITE
        [HttpGet]
        public async Task<ActionResult<IEnumerable<P_UNITE>>> GetP_UNITE([FromQuery] int page = 1, [FromQuery] int pageSize = 100)
        {
            int skip = (page - 1) * pageSize; pageSize = Math.Min(pageSize, 500); setDB(); return await _db.P_UNITE.AsNoTracking().Skip(skip).Take(pageSize).ToListAsync();
        }
