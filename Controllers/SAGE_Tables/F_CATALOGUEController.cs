...
namespace BusinessWeb.Controllers.SAGE_Tables
{
    ...
    public partial class F_CATALOGUEController : ControllerBase
    {
        ...
        // GET: api/F_CATALOGUE
        [HttpGet]
        public async Task<ActionResult<IEnumerable<F_CATALOGUE>>> GetF_CATALOGUE([FromQuery] int page = 1, [FromQuery] int pageSize = 100)
        {
            int skip = (page - 1) * pageSize; pageSize = Math.Min(pageSize, 500); setDB(); return await _db.F_CATALOGUE.AsNoTracking().Skip(skip).Take(pageSize).ToListAsync();
        }
