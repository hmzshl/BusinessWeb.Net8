...
namespace BusinessWeb.Controllers.SAGE_Tables
{
    ...
    public class API_V_MARGEController : ControllerBase
    {
        public DB _db = new DB(); BusinessWebDBContext _sdb = new BusinessWebDBContext();  Helpers fn = new Helpers();
        public API_V_MARGEController(BusinessWebDBContext sdb)
        {
            _sdb = sdb;
        }
		...
		{
			setDB(); var dt = _db.API_V_MARGE; var rs = await dt.Where(a => a.DO_Date >= DateDebut && a.DO_Date <= DateFin).ToListAsync(); dt = null; return rs; 
		}
